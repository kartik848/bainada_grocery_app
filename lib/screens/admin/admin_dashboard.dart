...
class _AdminDashboardState extends State<AdminDashboard> {
              ...
              children: [
                ...
                SizedBox(
                  ...
                  child: ListView.builder(
                    ...
                    itemCount: productProvider.categories.length + 2,
                    itemBuilder: (ctx, i) {
                      ...
                      if (i == productProvider.categories.length + 1) {
                        return Padding(
                          padding: const EdgeInsets.only(right: 6),
                          child: ActionChip(
                            avatar: const Icon(Icons.card_giftcard_rounded, size: 16, color: Color(0xFFD97706)),
                            label: const Text('🎁 Daily Offers', style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold, color: Color(0xFFD97706))),
                            backgroundColor: const Color(0xFFFEF3C7),
                            side: const BorderSide(color: Color(0xFFFCD34D)),
                            onPressed: () => _showSalesmanOffersManagerMobile(context),
                          ),
                        );
                      }
  ...
  void _showSalesmanOffersManagerMobile(BuildContext context) {
    showModalBottomSheet(
      context: context,
      isScrollControlled: true,
      backgroundColor: Colors.white,
        ...
        return StatefulBuilder(
          builder: (bottomSheetCtx, setSheetState) {
            final productProvider =
                Provider.of<ProductProvider>(bottomSheetCtx);
            final allProds = productProvider.allProducts;
            final displayed = searchQuery.trim().isEmpty
                ? allProds
                : allProds.where((p) {
            ...
            return Container(
              ...
              child: Column(
                ...
                children: [
                  ...
                  const Text(
                    'Set daily cash incentive for salesmen (e.g. ₹5/carton on Tata Salt)',
                    style: TextStyle(
                        fontSize: 12, color: AppColors.textSecondary),
                  ),
                    ...
                    child: displayed.isEmpty
                        ...
                        : ListView.separated(
                            ...
                            separatorBuilder: (_, __) =>
                                const SizedBox(height: 8),
                            itemBuilder: (_, idx) {
                              final prod = displayed[idx];
                              return _MobileSalesmanOfferTile(
                                product: prod,
                                onSave: (incentive, note, active) async {
                                  ...
                                  if (context.mounted) {
                                    ScaffoldMessenger.of(context).showSnackBar(
                                      SnackBar(
                                        content: Text(
                                          active
                                              ? 'Offer saved for "${prod.name}": ₹${incentive.toStringAsFixed(0)}/${prod.unit}'
                                              : 'Offer turned off for "${prod.name}"',
                                        ),
...
class _MobileSalesmanOfferTile extends StatefulWidget {
  final ProductModel product;
  final Future<void> Function(double incentive, String? note, bool isActive)
      onSave;

  const _MobileSalesmanOfferTile(
      {required this.product, required this.onSave});

  @override
  State<_MobileSalesmanOfferTile> createState() =>
      _MobileSalesmanOfferTileState();
}

class _MobileSalesmanOfferTileState extends State<_MobileSalesmanOfferTile> {
  late TextEditingController _incCtrl;
  ...
  void initState() {
    super.initState();
    _incCtrl = TextEditingController(
    ...
    _noteCtrl = TextEditingController(
      text: widget.product.salesmanOfferNote ??
          'Aaj 1 ${widget.product.unit} bechne par ₹5 offer!',
    );
    _active = widget.product.isSalesmanOfferActive;
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      padding: const EdgeInsets.all(10),
      decoration: BoxDecoration(
        color: _active ? const Color(0xFFFFFBEB) : Colors.white,
        borderRadius: BorderRadius.circular(8),
        border: Border.all(
          color:
              _active ? const Color(0xFFFCD34D) : const Color(0xFFE2E8F0),
        ),
      ),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      'Rate: ₹${prod.unitPriceWithGst.toStringAsFixed(2)} / ${prod.unit} (including tax)',
                      style: const TextStyle(
                          fontSize: 11,
              ...
              children: [
                ...
                ElevatedButton(
                  onPressed: _saving
                      ...
                      : () async {
                          final inc =
                              double.tryParse(_incCtrl.text.trim()) ?? 0.0;
                          setState(() => _saving = true);
                          await widget.onSave(
                              inc, _noteCtrl.text.trim(), _active);
